...
export async function testEnvironment(
  ...
  const apiKey =
    ...
    asString(parseObject(config.env).MINIMAX_API_KEY, "") ||
  ...
  if (!apiKey) {
    checks.push({
      ...
      hint: "Set adapterConfig.apiKey or set MINIMAX_API_KEY environment variable.",
    });
  } else {
    checks.push({
  ...
  if (!model) {
    checks.push({
      code: "minimax_model_required",
      level: "error",
      message: "No model configured.",
      hint: 'Set adapterConfig.model (e.g. "MiniMax-Text-01").',
    });
  } else {
    checks.push({
    ...
    try {
      ...
      if (response.ok) {
        checks.push({
          ...
          level: "info",
          message: "MiniMax API probe succeeded.",
        });
      } else {
        ...
        checks.push({
          ...
          level: "warn",
          message: `MiniMax API returned HTTP ${response.status}.`,
