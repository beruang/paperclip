...
import { parseMinimaxResponse } from "./parse.js";

const DEFAULT_API_BASE_URL = "https://api.minimax.chat/v1";
...
export async function execute(
  ...
  const model = asString(config.model, "MiniMax-Text-01");
  ...
  const apiKey =
    ...
    asString(envConfig.MINIMAX_API_KEY, "") ||
  ...
  if (!apiKey) {
    return {
      ...
      timedOut: false,
      errorMessage:
        "MiniMax API key is required. Set it via adapterConfig.apiKey or MINIMAX_API_KEY environment variable.",
    };
  }
  ...
  try {
    ...
    if (!response.ok) {
      return {
        ...
        signal: null,
        timedOut: false,
        errorMessage: `MiniMax API returned HTTP ${response.status}: ${responseText.slice(0, 500)}`,
        resultJson: { statusCode: response.status, body: responseText },
      };
    }

    const parsed = parseMinimaxResponse(responseText);
