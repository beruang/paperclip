...
import { parseBytesPlusResponse } from "./parse.js";
...
export async function execute(
  ...
  const apiKey =
    ...
    asString(envConfig.BYTESPLUS_API_KEY, "") ||
  ...
  if (!apiKey) {
    return {
      ...
      timedOut: false,
      errorMessage:
        "BytesPlus API key is required. Set it via adapterConfig.apiKey or BYTESPLUS_API_KEY environment variable.",
    };
  }
  ...
  if (!model) {
    return {
      ...
      signal: null,
      timedOut: false,
      errorMessage: "BytesPlus model/endpoint ID is required. Set adapterConfig.model.",
    };
  }
  ...
  try {
    ...
    if (!response.ok) {
      return {
        ...
        signal: null,
        timedOut: false,
        errorMessage: `BytesPlus API returned HTTP ${response.status}: ${responseText.slice(0, 500)}`,
        resultJson: { statusCode: response.status, body: responseText },
      };
    }

    const parsed = parseBytesPlusResponse(responseText);
